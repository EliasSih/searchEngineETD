...
def process_file(file):
    ...
    outputFile = file
    file = "./MetaData//" + file
    return Extract(file, outputFile)

def main():
    with concurrent.futures.ThreadPoolExecutor() as executor:
        txtfiles = [file for file in os.listdir("./MetaData") if file.endswith('.xml')]
        results = list(executor.map(process_file, txtfiles))
        print(txtfiles)
        print('Total num of docs', sum(results))
